...
def get_form(row):
    ...
    if is_valid(date):
        ...
        if type(date) is str:
            try:
                current_form['date'] = datetime.strptime(
                    date, TWO_DIGIT_YEAR_DATE_FORMAT).date()
            except:
                current_form['date'] = datetime.strptime(
                    date, FOUR_DIGIT_YEAR_DATE_FORMAT).date()
    ...
    question_list = [
        field for field in field_list if re.match(field_pattern, field)]

    formatted_question_list = [format_question(
...
def get_residents(row):
    ...
    for resident_index in range(number_of_residents):
        ...
        try:
            ...
            current_resident['mobility_restriction'] = find_marked_option(
                row, resident_index, range(3, 9))
            current_resident['sensory_impairment'] = find_marked_option(
                row, resident_index, range(9, 12))
            current_resident['mental_disorder'] = find_marked_option(
                row, resident_index, range(12, 15))
            current_resident['health_injury'] = find_marked_option(
                row, resident_index, range(15, 18))
            current_resident['education'] = find_marked_option(
                row, resident_index, range(18, 23))
